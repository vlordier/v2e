...
try:
    ...
    logger.info(
        f"{e}: Gooey GUI builder not available, "
...
def main():
    ...
    if synthetic_input is not None and input_file is not None:
        logger.error(
            f"Both input_file {input_file} and synthetic_input {synthetic_input} are specified - you can only specify one of them"
        )
        v2e_quit(1)
    ...
    if synthetic_input is not None:
        try:
            ...
            vid_path = os.path.join(output_folder, vid_orig) if vid_orig is not None else None
            ...
            synthetic_input_next_frame_method = synthetic_input_class.next_frame
            synthetic_input_total_frames_method = synthetic_input_class.total_frames
    ...
    if (
        input_start_time is not None
        and input_stop_time is not None
    ...
    if not disable_slomo and auto_timestamp_resolution is False and timestamp_resolution is None:
        logger.error(
    ...
    if synthetic_input is None:
        ...
        if srcFps == 0:
            logger.error(
                f"source {input_file} fps is 0; v2e needs to have a timescale for input video"
            )
            v2e_quit()
        ...
        start_frame = (
            int(srcNumFrames * (input_start_time / srcTotalDuration)) if input_start_time else 0
        )
        stop_frame = (
            int(srcNumFrames * (input_stop_time / srcTotalDuration))
            if input_stop_time
            else srcNumFrames - 1
        )
        ...
        srcFrameIntervalS = (1.0 / srcFps) / input_slowmotion_factor
        ...
        if disable_slomo:
            ...
            if slowdown_factor < NO_SLOWDOWN:
                ...
                logger.warning(
                    f"timestamp resolution={timestamp_resolution}s is >= source "
                    f"frame interval={srcFrameIntervalS}s, will not upsample"
                )
            ...
            if slomoTimestampResolutionS > timestamp_resolution:
                logger.warning(
                    f"Upsampled src frame intervals of {slomoTimestampResolutionS}s is larger than\n "
                    f"the desired DVS timestamp resolution of {timestamp_resolution}s"
                )
    ...
    if not synthetic_input and not auto_timestamp_resolution:
        ...
        if exposure_mode == ExposureMode.DURATION and dvsFps > (1 / slomoTimestampResolutionS):
            logger.warning(
                ...
                f"the effective DVS frame rate of {1 / slomoTimestampResolutionS}Hz; "
                "DVS video will have blank frames"
            )

    if not synthetic_input:
        logger.info(
            f"Source video {input_file} has total {srcNumFrames} frames with total duration {eng(srcTotalDuration)}s. "
            f"\nSource video is {eng(srcFps)}fps with slowmotion_factor {eng(input_slowmotion_factor)} "
            f"(frame interval {eng(srcFrameIntervalS)}s),"
            f"\nWill convert {stop_frame - start_frame + 1} frames {start_frame} to {stop_frame}\n"
            f"(From {start_time}s to {stop_time}s, duration {stop_time - start_time}s)"
        )

        if exposure_mode == ExposureMode.DURATION:
            dvsNumFrames = np.floor(dvsFps * srcDurationToBeProcessed / input_slowmotion_factor)
            ...
            logger.info(
                ...
                f"\nDVS video will have {dvsNumFrames} frames with duration {eng(dvsDuration)}s "
                f"and playback duration {eng(dvsPlaybackDuration)}s\n"
            )
            ...
            logger.info(
                f"v2e DVS video will have constant-count frames with {exposure_val} events), "
            )

    # check one more time that we have an output width and height
    if output_width is None or output_height is None:
        logger.error(
            "Either or both of output_width or output_height is None,\n"
    ...
    if synthetic_input_next_frame_method is not None:
        ...
        num_frames += 1
        i = 0
        with tqdm(total=synthetic_input_instance.total_frames(), desc="dvs", unit="fr") as pbar:
            with torch.no_grad():
        ...
        if start_frame > 0:
            ...
            for i in tqdm(range(start_frame), unit="fr", desc="src"):
                ...
                if not ret:
                    raise ValueError(
                        "something wrong, got to end of file before reaching start_frame"
                    )
        ...
        with TemporaryDirectory() as source_frames_dir:
            ...
            logger.info(
                f"Input video {input_file} has W={inputWidth} x H={inputHeight} frames each with {inputChannels} channels"
            )
            ...
            if (output_width is None) and (output_height is None):
                ...
                logger.warning(
                    f"output size ({output_width}x{output_height}) was set automatically to "
            ...
            for inputFrameIndex in tqdm(
                range(srcNumFramesToBeProccessed), desc="rgb2luma", unit="fr"
            ):
                # read frame
                ...
                if (
                    ...
                    inputVideoFrame = cv2.resize(
                        src=inputVideoFrame, dsize=dim, fx=fx, fy=fy, interpolation=cv2.INTER_AREA
                    )
                ...
                save_path = os.path.join(source_frames_dir, str(inputFrameIndex).zfill(8) + ".npy")
            ...
            with TemporaryDirectory() as interpFramesFolder:
                ...
                if slomo is not None and (
                    ...
                    interpTimes, avgUpsamplingFactor = slomo.interpolate(
                        source_frames_dir, interpFramesFolder, (output_width, output_height)
                    )
                    avgTs = srcFrameIntervalS / avgUpsamplingFactor
                    logger.info(
                        f"SloMo average upsampling factor={avgUpsamplingFactor:5.2f}; "
                        f"average DVS timestamp resolution={eng(avgTs)}s"
                    )
                    ...
                    logger.info(
                        f"*** Stage 2/3:turning npy frame files to png from {source_frames_dir}"
                    )
                    interpFramesFilenames = []
                    n = 0
                    src_files = sorted(glob.glob(f"{source_frames_dir}" + "/*.npy"))
                    for frame_idx, src_file_path in tqdm(
                        enumerate(src_files), desc="npy2png", unit="fr"
                    ):
                        src_frame = np.load(src_file_path)
                        tgt_file_path = os.path.join(interpFramesFolder, str(frame_idx) + ".png")
