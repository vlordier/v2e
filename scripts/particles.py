...
class particles(
    ...
    def __init__(
        ...
        )  # note floating point roundoff can produce small eps on top of step
        ...
        logger.info(
            f"speed(pixels/s): {self.speed_pps_min} to {self.speed_pps_max}\n"
    ...
    def cleanup(self):
        logger.info(
            f"particles() generated {self.particle_count:,} particles in {self.time:.3f}s"
        )
    ...
    def next_frame(self) -> tuple[np.ndarray | None, float]:
        ...
        if self.frame_number >= len(self.times):
            ...
            logger.info(
                f"finished after {self.frame_number} frames having made {self.particle_count} particles"
            )
