...
def draw_frame(height, width, t, bg, contrast, radius, freq):  # make spots
    ...
    if t > spots.TOTAL_TIME_S / 2:  # moving
        ...
        color = gray if np.abs(sine) < 0.5 else (bright if sine > 0.5 else dark)
...
def draw_square(frame, color, radius, x, y):
    rr, cc = draw.rectangle(
        (int(y - radius), int(x - radius)), (int(y + radius), int(x + radius))
    )
    frame[rr, cc] = color
...
class spots(
    ...
    def __init__(
        ...
        self.contrast: float = (
            args.contrast
        )  # spot contrast - compare with pos_thres and neg_thres and sigma_thr,
    ...
    def next_frame(self) -> tuple[np.ndarray | None, float]:
        ...
        if (
            ...
            and time == 0
            or time > self.last_frame_written_time + 1.0 / 1000.0
        ):
...
if __name__ == "__main__":
    ...
    with tqdm(
        total=m.total_frames(), desc="simulation", unit="fr"
