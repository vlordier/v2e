...
class DDD20SimpleReader:
    ...
    def __init__(self, fname, rotate180=True):
        ...
        logger.debug("group dvs contains following keys")
        for key in dvsGroup.keys():
            logger.debug(key)

        logger.debug("group dvs contains following items")
        ...
        logger.info(
            f"{fname} has {self.numPackets} packets with start time {self.firstTimeS:7.2f}s and end time {self.lastTimeS:7.2f}s (duration {self.durationS:8.1f}s)"
        )
    ...
    def search(self, timeS):
        ...
        for k in tqdm(
            range(self.firstPacketNumber, self.numPackets), unit="packet", desc="ddd-h5-search"
        ):
    ...
    def readEntire(self, startTimeS=None, stopTimeS=None):
        ...
        for k in tqdm(range(start, stop), desc="read", unit="packet"):
            ...
            if d["etype"] == "special_event":
                ...
                if any(d["data"] == 0):
                    logger.debug("ts reset detected, setting offset %s", timestamp)
                    t_offset += current
            ...
            if d["etype"] == "frame_event":
                ...
                data = np.array(
                    [(ts, frame)],
                    dtype=np.dtype([("ts", np.float64), ("frame", np.uint8, frame.shape)]),
                )
                frames.append(data)
...
class HDF5Stream(mp.Process):
    ...
    def _init_count(self, offset={}):
        ...
        self.size = {
            k: len(self.f[k]["data"]) - v * CHUNK_SIZE for k, v in self.block_offset.items()
        }
    ...
    def _init_time(self):
        ...
        for k in self.tables:
            ...
            while b > self.block_offset[k] and self.f[k]["timestamp"][b * CHUNK_SIZE] == 0:
                b -= 1
            logger.debug("%s final block: %s", k, b)
    ...
    def _bsearch_by_timestamp(self, k, t):
        ...
        logger.debug("searching %s %s", k, t)
        while True:
            if r - l < 2:
                logger.debug("selecting block %s", l)
                return l * CHUNK_SIZE
