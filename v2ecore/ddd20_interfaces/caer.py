...
class Controller:
        ...
        if len(cmd_parts) > self.max_cmd_parts:
            ...
            if cmd_parts[self.cmd_part_action] != None:
                ...
                if action_code == self.NODE_EXISTS:
                    ...
                    databuffer[10 : 10 + node_length] = self.cmd_parts[
                        self.cmd_part_node
                    ]
                ...
                if action_code == self.PUT:
                    ...
                    databuffer[10 : 10 + node_length] = str(
                        cmd_parts[self.cmd_part_node]
                    )
                    databuffer[10 + node_length : 10 + node_length + key_length] = str(
                        cmd_parts[self.cmd_part_key]
                    )
                    databuffer[
                        10 + node_length + key_length : 10
                        + node_length
                        + key_length
                        + value_length
    ...
    def set_aps(self, name, dtype, value):
        self.send_command(
            "put /1/1-DAVISFX3/%s/aps/ %s %s %s" % (SENSOR, name, dtype, value)
        )
