...
class Monitor(mp.Process):
    def __init__(self, bufsize=2048):
        ...
        logger.info("opened connection: %s", self.hdata)
...
class Controller:
    def __init__(self):
        ...
        try:
            ...
            logger.error("Failed to create socket %s", msg)
            quit()
    ...
    def parse_command(self, command):
        ...
        if len(cmd_parts) > self.max_cmd_parts:
            logger.error("command is made up of too many parts")
            return
        else:
            if cmd_parts[self.cmd_part_action] is not None:
                for i in range(len(self.actions)):
                    if cmd_parts[self.cmd_part_action] == self.actions[i][0]:
                        action_code = self.actions[i][2]
                if action_code == -1:
                    logger.warning("Please specify an action to perform as: get/put..")
                    return
                ...
                if action_code == self.PUT:
                    ...
                    databuffer[10 + node_length : 10 + node_length + key_length] = str(
                        cmd_parts[self.cmd_part_key]
                    )
                    databuffer[
                        10 + node_length + key_length : 10 + node_length + key_length + value_length
                    ] = str(cmd_parts[self.cmd_part_value])
...
if __name__ == "__main__":
    ...
    exposure = ExposureCtl(
        cutoff_bot=80, cutoff_top=0
    )  # set for upside down camera where sky will be at bottom
    while True:
        packet = aer.get()
        if not packet:
            continue
        ...
        if evts is not False:
            logger.debug("received events: %s", evts)
