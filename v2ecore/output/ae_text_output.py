...
class DVSTextOutput:
    ...
    def close(self):
        if self.file:
            logger.info(
                f"Closing {self.filepath} after writing {EngNumber(self.numEventsWritten)} events"
            )
            self.file.close()
            self.file = None
    ...
    def _writeHeader(self):
        ...
        time = f"# Creation time: System.currentTimeMillis() {int(time.time() * 1000.0)}\n"
    ...
    def appendEvents(self, events: np.ndarray, signnoise_label: np.ndarray = None):
        ...
        if signnoise_label is None:
            lines = np.column_stack([t, x, y, p]).astype(str)
            self.file.writelines(" ".join(row) + "\n" for row in lines)
        else:
            labels = signnoise_label.astype(np.int32)
            lines = np.column_stack([t, x, y, p, labels]).astype(str)
            self.file.writelines(" ".join(row) + "\n" for row in lines)
