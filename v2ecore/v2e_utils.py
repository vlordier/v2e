...
def make_output_folder(output_folder_base, suffix_counter, overwrite, unique_output_folder) -> str:
    ...
    output_folder = (
        output_folder_base + f"-{suffix_counter}" if suffix_counter > 0 else output_folder_base
    )
...
def set_output_folder(
    ...
    if output_in_place:
        ...
        output_folder = make_output_folder(output_folder, 0, overwrite, unique_output_folder)
...
def histogram_events_in_time_bins(events, start=0, stop=3.5, time_bin_ms=50, polarity=None):
    ...
    for i in range(bin_num - 1):
        condition = np.logical_and(events[:, 0] >= ticks[i], events[:, 0] < ticks[i + 1])
        if polarity:
