...
def v2e_args(parser):
    ...
    outGroupGeneral.add_argument(
        "--overwrite",
        action="store_true",
        help="overwrites files in existing folder (checks existence of non-empty output_folder).",
    )
    outGroupGeneral.add_argument(
    ...
    camAction.add_argument(
        "--dvs128", action="store_true", help="Set size for 128x128 DVS (DVS128)"
    )
    camAction.add_argument(
        "--dvs240", action="store_true", help="Set size for 240x180 DVS (DAVIS240)"
    )
    camAction.add_argument(
        "--dvs346", action="store_true", help="Set size for 346x260 DVS (DAVIS346)"
    )
    camAction.add_argument(
        "--dvs640", action="store_true", help="Set size for 640x480 DVS (DAVIS640)"
    )
    camAction.add_argument(
        "--dvs1024",
        action="store_true",
    ...
    sloMoGroup.add_argument(
        "--vid_slomo",
        type=none_or_str,
        default="video_slomo.avi",
        help="Output slomo of src video slowed down by slowdown_factor."
        "Specify emtpy string or 'None' to skip output.",
    )

    sloMoGroup.add_argument(
        "--slomo_stats_plot", action="store_true", help="show a plot of slomo statistics"
    )
    ...
    dvsEventOutputGroup.add_argument(
        ...
        type=output_file_check,
        default=None,
        help="Output DV AEDAT-4.0 event file To suppress, supply argument None. ",
    )
    dvsEventOutputGroup.add_argument(
        "--dvs_text",
        type=output_file_check,
    ...
    csdvs.add_argument(
        ...
        type=float,
        default=None,
        help="space constant of surround in pixels, None to disable.  "
    ...
    csdvs.add_argument(
        ...
        type=float,
        default=None,
        help="time constant of photoreceptor center of diffuser in ms, or 0 to disable for instantaneous surround. "
    ...
    scidvs_group.add_argument(
        "--scidvs",
        action="store_true",
        help="Simulate proposed SCIDVS pixel with nonlinear adapatation and high gain",
    )
...
def v2e_check_dvs_exposure_args(args):
    if not args.dvs_exposure:
        raise ValueError("define --dvs_exposure method. See extended usage.")
    ...
    try:
        ...
        raise ValueError(
            f"dvs_exposure first parameter '{dvs_exposure[0]}' must be 'duration','count', "
            " 'area_count' or 'source'"
        )
    ...
    if exposure_mode == ExposureMode.AREA_COUNT and not len(dvs_exposure) == 3:
        ...
        raise ValueError(
            "duration or count argument needs two parameters, e.g. 'duration 0.01' or 'count 3000'"
        )
    ...
    if not exposure_mode == ExposureMode.AREA_COUNT:
        try:
            ...
            raise ValueError(
                "dvs_exposure second parameter must be a number, either duration or event count"
            )
    else:
        try:
            ...
            raise ValueError(
                "area_count must be N M, where N is event count and M is area dimension in pixels"
            )
    s = f"DVS frame expsosure mode {exposure_mode}"
    if exposure_mode == ExposureMode.DURATION:
        s = s + f": frame rate {1.0 / exposure_val}"
    elif exposure_mode == ExposureMode.COUNT:
        s = s + f": {exposure_val} events/frame"
    elif exposure_mode == ExposureMode.AREA_COUNT:
        s = s + f": {exposure_val} events per {area_dimension}x{area_dimension} pixel area"
