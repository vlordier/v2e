...
from v2ecore.v2e_utils import checkAddSuffix, hist2d_numba_seq, v2e_quit, video_writer
...
class EventRenderer:
    ...
    def cleanup(self):
        if self.video_output_file is not None:
            logger.info(
                f"Closing DVS video output file {self.video_output_file_name} "
                f"after writing {self.numFramesWritten} frames"
            )
    ...
    def _check_outputs_open(self):
        ...
        if self.output_path and type(self.video_output_file_name) is str:
            ...
            s = f"# frame times for {self.video_output_file_name}\n# frame# time(s)\n"
    ...
    def render_events_to_frames(
        ...
        numEvents = len(ts)
        histrange = np.asarray([(0, v) for v in (self.height, self.width)], dtype=np.int64)
        ...
        while not doneWithTheseEvents:
            ...
            if self.exposure_mode == ExposureMode.DURATION:
                ...
                self.area_counts, end = compute_area_counts(
                    event_arr, self.area_counts, self.area_count, self.area_dimension, start
                )
            elif self.exposure_mode == ExposureMode.SOURCE:
                start = 0
                end = numEvents
            ...
            if not doneWithTheseEvents or self.exposure_mode == ExposureMode.SOURCE:
                ...
                if self.video_output_file:
                    ...
                    self.frame_times_output_file.write(f"{self.numFramesWritten}\t{t:10.6f}\n")
    ...
    def accumulate_event_frame(self, events, histrange):
        ...
        self.currentFrame = np.clip(
            self.currentFrame + (img_on - img_off), -self.full_scale_count, self.full_scale_count
        )
