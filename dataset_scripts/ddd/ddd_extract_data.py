...
parser = argparse.ArgumentParser(
    description="Extract data from DDD recording",
...
parser.add_argument(
    "-o", "--output_folder", type=str, required=True, help="folder to store outputs"
)
parser.add_argument(
    "--start_time",
    type=float,
...
parser.add_argument(
    "--stop_time", type=float, default=None, help="stop point of video stream"
)
parser.add_argument("--rotate180", action="store_true", help="rotate output 180 deg")
parser.add_argument(
    "--overwrite",
    action="store_true",
    help="overwrites files in existing folder (checks existance of non-empty output_folder)",
)
parser.add_argument(
    "--dvs240",
    action="store_true",
...
if args.dvs240 == False and args.dvs346 == False:
    logger.error(
        "Specify either --dvs240 for DDD17 recording or --dvs346 for DDD20 recording."
    )
    quit(1)

# Set output width and height based on the arguments
output_width, output_height = set_output_dimension(
    None, None, False, args.dvs240, args.dvs346, False, False, logger
)
