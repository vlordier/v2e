...
from v2ecore.v2e_args import v2e_args, v2e_check_dvs_exposure_args, write_args_info
...
if __name__ == "__main__":
    ...
    if f:
        logger.error(f"output folder {os.path.abspath(output_folder)} already exists\n it holds files {f}\n - use --overwrite")
        quit()

    if not os.path.exists(output_folder):
        logger.info(f"making output folder {output_folder}")
    ...
    if not exposure_mode==ExposureMode.DURATION:
        raise ValueError(f"only dvs_exposure=duration is currently supported (mode {exposure_mode} not allowed)")
    ...
    for i in tqdm(range(startPacket, stopPacket),desc="v2e-ddd20",unit="packet"):
            ...
            if frame0 is not None and frame1 is not None:
                with TemporaryDirectory() as interpFramesFolder:
                    ...
                    for i in range(n - 1):  # for each interpolated frame up to last; use n-1 because we get last interpolated frame as first frame next time
                        ...
                        if newEvents is not None: events = np.append(events, newEvents, axis=0)
        ...
        if numpy_output and platform.startswith("linux"):
            ...
            if usageRSS_kB>memoryLimit/1024:
                logger.warning(f"\n*** memory usage (RSS) is {EngNumber(1024*usageRSS_kB)}B")
                memoryLimit+=1e9
    ...
    if output_folder and numpy_output:
        ...
        logger.info(f"saved numpy files with real and v2e events to {output_folder}")
    ...
    logger.info(f"done processing {num_frames} frames in {EngNumber(totalTime)}s ({throughputStr})\n see output folder {output_folder}")
    logger.info(f"generated total {EngNumber(emulator.num_events_total)} events ({EngNumber(emulator.num_events_on)} on, {EngNumber(emulator.num_events_off)} off)")
    logger.info(f"avg event rate {EngNumber(emulator.num_events_total/srcDurationToBeProcessed)}Hz ({EngNumber(emulator.num_events_on/srcDurationToBeProcessed)}Hz on, {EngNumber(emulator.num_events_off/srcDurationToBeProcessed)}Hz off)")
    try:
