...
if __name__ == "__main__":
    ...
    if f:
        logger.error(
            f"output folder {os.path.abspath(output_folder)} already exists\n it holds files {f}\n - use --overwrite"
        )
        quit()
    ...
    if exposure_mode is not ExposureMode.DURATION:
        raise ValueError(
    ...
    startPacket = (
        davisData.search(timeS=start_time)
        if start_time
    ...
    stopPacket = (
        davisData.search(timeS=stop_time) if stop_time else davisData.numPackets - 1
    )
    ...
    logger.info(
        f"done processing {num_frames} frames in {EngNumber(totalTime)}s ({throughputStr})\n see output folder {output_folder}"
    )
    logger.info(
        f"generated total {EngNumber(emulator.num_events_total)} events ({EngNumber(emulator.num_events_on)} on, {EngNumber(emulator.num_events_off)} off)"
    )
    logger.info(
        f"avg event rate {EngNumber(emulator.num_events_total / srcDurationToBeProcessed)}Hz ({EngNumber(emulator.num_events_on / srcDurationToBeProcessed)}Hz on, {EngNumber(emulator.num_events_off / srcDurationToBeProcessed)}Hz off)"
    )
    try:
