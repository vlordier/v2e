...
parser = argparse.ArgumentParser(
    description="ddd_find_thresholds.py: generate simulated DVS events from video with sweep of thresholds to compare with real DVS to find optimal thresholds.",
    epilog="Run with no --input to open file dialog.\nIf slomo.avi aleady exists in output_folder, script will load frames from there rather than regenerating them with SuperSloMo.",
    allow_abbrev=True,
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument(
    "--start", type=float, default=0.0, help="start point of video stream"
)
parser.add_argument(
    "--stop", type=float, default=5.0, help="stop point of video stream"
)
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="path to where output is stored (usually previous output folder from ddd_v2e.py)",
)
parser.add_argument(
    "--slowdown_factor", type=int, default=10, help="slow motion factor"
)
parser.add_argument(
    "--slomo_model",
    type=str,
    default="input/SuperSloMo39.ckpt",
...
parser.add_argument(
    ...
    nargs=2,
    default=None,
    help="x ROI, two integers, e.g. 10 20; if None, entire address space",
)
parser.add_argument(
    "--y",
    type=int,
    nargs=2,
    default=None,
    help="y ROI, two integers, e.g. 40 50; if None, entire address space",
)
parser.add_argument(
    "--rotate180",
    type=bool,
...
if __name__ == "__main__":
    ...
    if args.x == None:
        args.x = tuple(0, DVS_WIDTH)
    if args.y == None:
        args.y = tuple(0, DVS_HEIGHT)
    ...
    if frames is None or dvsEvents is None:
        raise Exception("no frames or no events")
    ...
    with TemporaryDirectory() as interp_frames_dir:
        ...
        slomo.interpolate(
            images=frames["frame"], output_folder=interp_frames_dir
        )  # writes all frames to interp_frames_folder
        ...
        emulator = EventEmulator(
            output_folder=None, show_dvs_model_state=None
        )  # 'baseLogFrame'
        k = 0
        min_pos_diff = np.inf
        min_neg_diff = np.inf
        ...
        online, offline = ax.plot(
            thresholds, on_diffs, "g-", thresholds, off_diffs, "r-"
        )
        online.set_label("On")
