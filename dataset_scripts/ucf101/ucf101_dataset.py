...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, help="path of UCF-101 dataset"
    )
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="path to store output videos"
    )
    ...
    for i, action in enumerate(classes):
        ...
        input_ts = output_ts = np.linspace(
            0, num_frames / fps, num_frames, endpoint=False
        )

        with TemporaryDirectory() as dirname:
            print("tmp_dir: ", dirname)

            s = SuperSloMo(args.checkpoint, args.sf, dirname, video_path=output_path)
